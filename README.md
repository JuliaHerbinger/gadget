...
``` 
    ...
    |   |   ├── batchtools/      # Scripts used to create data for simulation experiments (Sec. 6.1-6.3, Sec. 5 and Appendix F)
...
2.  Create an experimental registry, add experiments and problem and run simulations via
    script `R/simulations/generate_data_*.R` where `*` stands for `cor` (Sec. 6.1), `higher_order` (Sec. 6.2), `pint` (Sec. 6.3) or `pint_eval` (Sec. 5 and App. F). Data produced by the scripts is stored in 
    the subfolder `data/batchtools/` as a separate registry.
    
3.  Prepare data for analysis by running the script `R/simulations/reduce_experiments_*.R` where `*` stands for `cor` (Sec. 6.1), `higher_order` (Sec. 6.2), `pint` (Sec. 6.3) or `pint_eval` (Sec. 5 and App. F).
